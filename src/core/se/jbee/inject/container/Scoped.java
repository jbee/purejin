...
public final class Scoped {
	 ...
	 */
	public static final Scope THREAD = new ThreadScope(new ThreadLocal<>(),
			APPLICATION);
	...
	private static final class ThreadScope implements Scope, Repository {
		...
		ThreadScope(ThreadLocal<Repository> threadRepository,
	...
	/**
	 * The 'synchronous'-{@link Repository} will be asked first passing a
	 * special resolver that will ask the 'asynchronous' repository when
	 * invoked. Thereby the repository originally bound will be asked once.
	 * Thereafter the result is stored in the synchronous repository.
	 *
	 * Both repositories will remember the resolved instance whereby the
	 * repository considered as the synchronous-repository will deliver a
	 * consistent image of the world as long as it exists.
	 *
	 * @author Jan Bernitt (jan@jbee.se)
	 */
	private static final class SnapshotRepository implements Repository {
		...
		public <T> T serve(int serialID, Dependency<? super T> dep,
				Provider<T> provider) throws UnresolvableDependency {
			return dest.serve(serialID, dep,
					new SnapshotingProvider<>(dep, serialID, provider, src));
		}
		...
		private static final class SnapshotingProvider<T>
			...
			SnapshotingProvider(Dependency<? super T> dep, int serialID,
					Provider<T> supplier, Repository src) {
				this.dependency = dep;
	...
	public static abstract class DependencyBasedScope
			implements Scope.SingletonScope {
	...
	private static final class TargetedDependencyTypeScope
			extends DependencyBasedScope {
	...
	private static final class TargetInstanceScope
			extends DependencyBasedScope {
		TargetInstanceScope() {
	...
	private static final class DependencyInstanceScope
			extends DependencyBasedScope {
