...
public final class EventPreferences implements Serializable {
	...
	public enum Flags {
		...
		/**
		 * Whether or not to synchronise after a multi-dispatch so that the call
		 * to the handler method completes when the dispatch is done. Default
		...
		/**
		 * Whether or not to use multi-dispatch for methods with return type
		 * {@link Boolean} or {@code boolean} and combine their result with OR
		 * operator. Default should be {@code false}.
		 */
		MULTI_DISPATCH_AGGREGATED,

		/* Exception Handling */
		/**
		 * Whether or not to return {@code null} or zero or {@code false} in
		 * case there is no handler for the event instead of throwing an
		 * {@link EventException}.
		 */
	 ...
	 * other method will be called until the call is complete. The effect is as
	 * if all method would have been marked {@code synchronized}. However, this
	...
	public EventPreferences(int maxConcurrentUsage, int ttl,
			EnumSet<Flags> flags) {
