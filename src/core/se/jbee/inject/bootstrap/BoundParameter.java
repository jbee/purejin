 ...
 *
 * @param <T> The {@link Type} of the parameter
 */
public final class BoundParameter<T> implements Parameter<T> {

	public enum ParameterType {
		INSTANCE, CONSTANT, EXTERNAL
	}
	...
	public static <T> BoundParameter<T> bind(Parameter<T> parameter) {
		...
		if (parameter instanceof Instance<?>) {
			...
			return new BoundParameter<>(ParameterType.INSTANCE, i.type(), i,
					null, Supply.instance(i));
		}
		...
		if (parameter instanceof Dependency<?>) {
			...
			return new BoundParameter<>(ParameterType.EXTERNAL, d.type(),
					d.instance, null, Supply.dependency(d));
		}
		throw new IllegalArgumentException(
				"Unknown parameter type:" + parameter);
	}
	...
	public static <T> Parameter<T> constant(Type<T> type, T constant) {
		return new BoundParameter<>(ParameterType.CONSTANT, type,
	...
	public static <T> Parameter<T> supplier(Type<T> type,
		...
		return new BoundParameter<>(ParameterType.EXTERNAL, type,
	...
	@SafeVarargs
	public static <E> BoundParameter<? extends E>[] bind(
	...
	public BoundParameter(ParameterType type, Type<T> asType,
			Instance<? extends T> instance, T value,
			Supplier<? extends T> supplier) {
		this.type = asType.rawType == Injector.class
	...
	public BoundParameter<T> external() {
		return new BoundParameter<>(ParameterType.EXTERNAL, asType, instance,
				value, supplier);
	}

	/**
	 * @param type The new type of this {@link BoundParameter}
